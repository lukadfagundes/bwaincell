...
async function handleList(interaction, userId) {
  ...
  if (notes.length === 0) {
    await interaction.editReply(
      "📝 You have no notes yet. Use `/notes add` to create one!",
    );
    return;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor("#00FF00")
    .setDescription(
      notes.map((note) => `**${note.title}** (ID: ${note.id})`).join("\n"),
    );
...
const embed = new EmbedBuilder()
  ...
  .setColor("#00FF00")
  .addFields(
    { name: "Task 1", value: "Buy groceries" },
    { name: "Task 2", value: "Call dentist" },
  );
