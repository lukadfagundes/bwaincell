...
export const InteractionScenarios = {
  taskAdd: (description: string, priority?: string) =>
  ...
  budgetAdd: (description: string, amount: number, category?: string) =>
    createMockInteraction({
      commandName: 'budget',
      subcommand: 'add',
      options: {
  ...
  scheduleAdd: (title: string, datetime: string, recurring?: boolean) =>
    createMockInteraction({
